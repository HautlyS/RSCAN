...
impl PointCloud {
    pub fn new() -> Self {
        Self { points: vec![], colors: vec![], normals: vec![] }
    }
    ...
    pub fn from_ply(path: &std::path::Path) -> Result<Self, String> {
        ...
        loop {
            ...
            if line.contains("red") { has_color = true; }
    ...
    pub fn remove_outliers(&mut self, k: usize, std_ratio: f32) {
        ...
        let distances: Vec<f32> = self.points.iter()
            .map(|p| ((p[0]-mean[0]).powi(2) + (p[1]-mean[1]).powi(2) + (p[2]-mean[2]).powi(2)).sqrt())
            .collect();
        
        let mean_dist = distances.iter().sum::<f32>() / distances.len() as f32;
        let std = (distances.iter().map(|d| (d - mean_dist).powi(2)).sum::<f32>() / distances.len() as f32).sqrt();
        let threshold = mean_dist + std_ratio * std;
        
        let mask: Vec<bool> = distances.iter().map(|d| *d < threshold).collect();
        self.points = self.points.iter().zip(&mask).filter(|(_, &m)| m).map(|(p, _)| *p).collect();
        if !self.colors.is_empty() {
            self.colors = self.colors.iter().zip(&mask).filter(|(_, &m)| m).map(|(c, _)| *c).collect();
        }
    }
}
